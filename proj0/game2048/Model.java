...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for(int col = 0; col < board.size(); col++){
            int lastMergeRow =  board.size();
            for(int row = board.size() - 2; row >= 0; row--){
                // tranform side

                int row0 = side.row(col, row, board.size());
                int col0 = side.col(col, row, board.size());
                Tile t = board.tile(col0, row0);
                if(t != null){
                    int newRow = row;
                    //check for furthest move
                    while( (newRow+1) < board.size() ){
                        int col1 = side.col(col, newRow+1, board.size());
                        int row1 = side.row(col, newRow+1, board.size());
                        if(board.tile(col1, row1 ) == null) newRow++;
                        else break;
                    }
                    //check for merge
                    if(newRow + 1 < board.size()){
                    ...
                    if (newRow != row) {
                        //int c = side.col(col0, newRow, board.size());
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        // 2. There are two adjacent tiles with the same value.
        // for every tile check its right and down adjacent tile
        // This time no empty space
        for(int col = 0; col < b.size(); col++){
            for(int row = 0; row < b.size(); row++){
                // right tile
                if(col + 1 < b.size()){
